...
void app_main(void) {
	...
	switch_event_queue =
		xQueueCreate(SWITCH_QUEUE_SIZE, sizeof(switch_event_t));
	...
	switch_config_t switch_cfg = {
		.pin = SWITCH_PIN_1,
		.active_low =
			true, // Assumes the switch pulls the pin to GND when closed
		.debounce_ms = 50};
	...
	esp_err_t integrator_status = init_queue_manager(
		button_event_queue, encoder_event_queue, espnow_event_queue,
		touch_event_queue, switch_event_queue, integrated_event_queue);
