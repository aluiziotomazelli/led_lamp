...
void app_main(void) {
	...
	led_controller_config_t led_cfg = {
		.data_gpio = LED_CONTROLLER_DATA_PIN,
		...
		.with_dma = true,
		.pixel_format =
			(led_color_component_format_t){.format = {.r_pos = 1,
													  .g_pos = 0,
													  .b_pos = 2,
													  .num_components = 3}}};
	...
	task_created = xTaskCreate(app_logic_task, "app_logic_task",
							   TASK_STACK_SIZE_APP_LOGIC, NULL, 4, NULL);
	configASSERT(task_created == pdPASS);
