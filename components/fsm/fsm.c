...
#include "fsm.h"
#include "led_controller.h"  // Para controle dos LEDs
// #include "integrated_events.h"  // Onde estão definidos os tipos de eventos

static const char *TAG = "FSM";

// Configurações default
#define FSM_DEFAULT_STACK_SIZE      4096
#define FSM_DEFAULT_PRIORITY        5
#define FSM_DEFAULT_TIMEOUT_MS      100
#define FSM_DEFAULT_MODE_TIMEOUT_MS 30000
...
typedef struct {
    ...
    fsm_config_t config;                ///< Configurações
...
esp_err_t fsm_init(QueueHandle_t queue_handle, const fsm_config_t *config) {
    ...
    if (config != NULL) {
        ...
        fsm_ctx.config.task_stack_size = FSM_DEFAULT_STACK_SIZE;
        fsm_ctx.config.task_priority = FSM_DEFAULT_PRIORITY;
        fsm_ctx.config.queue_timeout_ms = pdMS_TO_TICKS(FSM_DEFAULT_TIMEOUT_MS);
        fsm_ctx.config.mode_timeout_ms = FSM_DEFAULT_MODE_TIMEOUT_MS;
    }

    fsm_ctx.event_queue = queue_handle;
    
    // Inicializa estado inicial
    fsm_ctx.current_state = FSM_STATE_DISPLAY;
    fsm_ctx.previous_state = FSM_STATE_DISPLAY;
...
esp_err_t fsm_force_state(fsm_state_t new_state) {
    if (new_state >= FSM_STATE_SYSTEM_SETUP + 1) {
        return ESP_ERR_INVALID_ARG;
    }
...
//    ESP_LOGD(TAG, "Button event: %d in state %d", button_event->type, fsm_ctx.current_state);
    ...
    switch (fsm_ctx.current_state) {
        case FSM_STATE_DISPLAY:
            switch (button_event->type) {
                ...
                case BUTTON_DOUBLE_CLICK:
                    ...
                    fsm_transition_to_state(FSM_STATE_EFFECT_SELECT);
                    break;
                ...
                case BUTTON_LONG_CLICK:
                    ...
                    fsm_ctx.setup_param_index = 0;
                    fsm_ctx.setup_has_changes = false;
                    fsm_transition_to_state(FSM_STATE_EFFECT_SETUP);
                    break;
                    
                case BUTTON_VERY_LONG_CLICK:
                    // Entra no setup do sistema
                    fsm_transition_to_state(FSM_STATE_SYSTEM_SETUP);
                    break;
        ...
        case FSM_STATE_EFFECT_SELECT:
            switch (button_event->type) {
                case BUTTON_CLICK:
                    // Seleciona o efeito atual e volta para exibição
                    fsm_transition_to_state(FSM_STATE_DISPLAY);
                    fsm_update_led_display();
...
static void fsm_check_mode_timeout(uint32_t current_time) {
    if (fsm_ctx.current_state == FSM_STATE_DISPLAY) {
