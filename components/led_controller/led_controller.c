...
esp_err_t led_controller_init(const led_controller_config_t *config) {
    ...
    if (ret == ESP_ERR_INVALID_STATE) {
        ...
        // Continue, assuming the bus was initialized with a compatible configuration
    } else if (ret == ESP_OK) {
        s_spi_bus_was_initialized_by_us = true;
        ESP_LOGI(TAG, "SPI bus %d initialized successfully.", (int)config->spi_host);
    } else {
    ...
    led_strip_config_t strip_config = {
        .strip_gpio_num = config->spi_mosi_gpio,
...
esp_err_t led_controller_set_pixel_hsv(uint32_t index, uint16_t h, uint8_t s, uint8_t v) {
    ...
    ESP_RETURN_ON_FALSE(index < s_current_config.num_leds, ESP_ERR_INVALID_ARG, TAG, "Pixel index %" PRIu32 " out of bounds", index);
