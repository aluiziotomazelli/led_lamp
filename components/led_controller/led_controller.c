 ...
 *          Includes NVS integration, command handling, and system
 * configuration.
 *
 * @author Your Name
 * @date 2024-03-15
...
static void handle_command(const led_command_t *cmd) {
	...
	case LED_CMD_SET_EFFECT:
		if (cmd->value >= 0 && cmd->value < effects_count) {
			...
			ESP_LOGI(TAG, "Effect set to index: %d (%s)", current_effect_index,
	...
	case LED_CMD_SET_EFFECT_PARAM: {
		...
		if (current_effect->num_params > 0 &&
			...
			ESP_LOGI(TAG, "Param '%s' (#%d) set to: %d", param->name, param_idx,
					 param->value);
#if ESP_NOW_ENABLED && IS_MASTER
			send_espnow_command(cmd);
#endif
	...
	case LED_CMD_SET_STRIP_MODE:
		if (cmd->value == 1) { // Restricted mode (from switch open)
			...
			active_num_leds =
				NUM_LEDS - (g_led_offset_begin + g_led_offset_end);
	...
	case LED_CMD_SAVE_CONFIG:
		...
		// When effect setup is saved, it's a good time to save all static
		// parameters
		trigger_static_save();
		break;
 ...
 */
void led_controller_apply_nvs_data(const volatile_data_t *v_data,
								   const static_data_t *s_data) {
	...
	if (current_effect_index >= effects_count) {
		ESP_LOGW(TAG, "Saved effect index %d is out of bounds, resetting to 0.",
	...
	for (uint8_t i = 0; i < effects_count && i < NVS_NUM_EFFECTS; i++) {
		for (uint8_t j = 0;
			 j < effects[i]->num_params && j < NVS_MAX_PARAMS_PER_EFFECT; j++) {
...
QueueHandle_t led_controller_init(QueueHandle_t cmd_queue) {
	...
	brightness_save_timer =
		xTimerCreate("BrightnessTimer", pdMS_TO_TICKS(10000), pdFALSE,
					 (void *)0, brightness_save_callback);
	if (brightness_save_timer == NULL) {
		...
		if (should_run_effect) {
			if (current_brightness > 0) {
				...
				if (strip_data.mode == COLOR_MODE_HSV) {
					for (uint16_t i = 0; i < NUM_LEDS; i++) {
						uint8_t v =
							(pixel_buffer[i].hsv.v * current_brightness) / 255;
...
void led_controller_restore_current_effect_defaults(void) {
	...
	ESP_LOGI(TAG, "Restoring parameters for effect '%s' to default.",
			 effect->name);
...
void led_controller_factory_reset(void) {
	...
	trigger_static_save();
	trigger_volatile_save(); // Also save volatile state like brightness/effect
							 // index
 ...
 */
bool led_controller_is_on(void) { return is_on; }
 ...
 */
uint8_t led_controller_get_brightness(void) { return master_brightness; }
 ...
 */
uint8_t led_controller_get_effect_index(void) { return current_effect_index; }
