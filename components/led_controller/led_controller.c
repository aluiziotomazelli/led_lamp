...
// Temporary storage for parameters when in setup mode (for cancel functionality)
...
/**
 * @brief Saves the current parameters of the active effect into a temporary buffer.
 */
static void save_temp_params() {
    ...
    if (current_effect->num_params > 0) {
        ...
        if (temp_params) {
            memcpy(temp_params, current_effect->params, sizeof(effect_param_t) * current_effect->num_params);
        } else {
...
/**
 * @brief Restores the parameters of the active effect from the temporary buffer.
 */
static void restore_temp_params() {
    effect_t *current_effect = effects[current_effect_index];
    if (temp_params && current_effect->num_params > 0) {
        memcpy(current_effect->params, temp_params, sizeof(effect_param_t) * current_effect->num_params);
        free(temp_params);
        temp_params = NULL;
    }
}
...
static void handle_command(const led_command_t *cmd) {
    ...
    switch (cmd->cmd) {
        ...
        case LED_CMD_INC_EFFECT: {
            ...
            if (new_index < 0) new_index = effects_count - 1;
            if (new_index >= effects_count) new_index = 0;
            current_effect_index = new_index;
            current_param_index = 0; // Reset param index when changing effect
            ESP_LOGI(TAG, "Effect changed to: %s", effects[current_effect_index]->name);
            break;
        }
        ...
        case LED_CMD_INC_EFFECT_PARAM:
            if (current_effect->num_params > 0) {
                ...
                if (param->value > param->max_value) param->value = param->max_value;
                if (param->value < param->min_value) param->value = param->min_value;
        ...
        case LED_CMD_NEXT_EFFECT_PARAM:
            if (current_effect->num_params > 0) {
                ...
                ESP_LOGI(TAG, "Next param: %s", current_effect->params[current_param_index].name);
            }
            break;
...
QueueHandle_t led_controller_init(QueueHandle_t cmd_queue) {
    ...
    BaseType_t result = xTaskCreate(led_controller_task, "LED_CTRL_T", LED_CTRL_STACK_SIZE, NULL, LED_CTRL_TASK_PRIORITY, NULL);
    if (result != pdPASS) {
    ...
    while(1) {
        ...
        if (is_on) {
            ...
            for(uint16_t i=0; i<NUM_LEDS; i++) {
                pixel_buffer[i] = apply_brightness(pixel_buffer[i], master_brightness);
            }
        } else {
