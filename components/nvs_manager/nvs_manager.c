...
/**
 * @brief Loads default values for the ota_data_t struct
 *
 * @param[out] data Pointer to ota_data_t to populate with defaults
 */
static void load_ota_defaults(ota_data_t *data);
...
static void load_static_defaults(static_data_t *data) {
    ...
    data->correction_r = DEFAULT_RED_CORRECTION;
    data->correction_g = DEFAULT_GREEN_CORRECTION;
    data->correction_b = DEFAULT_BLUE_CORRECTION;
...
/**
 * @brief Load default values for OTA data
 */
static void load_ota_defaults(ota_data_t *data) {
    ESP_LOGI(TAG, "Loading default OTA data.");
    data->ota_mode_enabled = false;
    memset(data->wifi_ssid, 0, sizeof(data->wifi_ssid));
    memset(data->wifi_password, 0, sizeof(data->wifi_password));
}
...
esp_err_t nvs_manager_save_ota_data(const ota_data_t *data) {
    ...
    err = nvs_set_blob(nvs_handle, KEY_OTA_DATA, data, sizeof(ota_data_t));
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error writing OTA data to NVS: %s", esp_err_to_name(err));
    } else {
        err = nvs_commit(nvs_handle);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Error committing OTA data to NVS: %s", esp_err_to_name(err));
        } else {
            ESP_LOGI(TAG, "OTA data saved successfully.");
        }
    }
...
esp_err_t nvs_manager_load_ota_data(ota_data_t *data) {
    ...
    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ...
        load_ota_defaults(data);
        return ESP_ERR_NVS_NOT_FOUND; // Return specific error to signal defaults loaded
    } else if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error opening NVS handle: %s", esp_err_to_name(err));
        load_ota_defaults(data); // Load defaults on any other error for safety
        return err;
    }

    size_t required_size = sizeof(ota_data_t);
    err = nvs_get_blob(nvs_handle, KEY_OTA_DATA, data, &required_size);

    if (err == ESP_ERR_NVS_NOT_FOUND) {
        ESP_LOGW(TAG, "OTA data not found in NVS. Loading defaults.");
        load_ota_defaults(data);
        // This is not an error condition, we just loaded defaults
        err = ESP_OK;
    } else if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error reading OTA data from NVS: %s", esp_err_to_name(err));
    } else {
        ESP_LOGI(TAG, "OTA data loaded successfully from NVS.");
    }
