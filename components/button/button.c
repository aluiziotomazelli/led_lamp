	...
	case BUTTON_DEBOUNCE_RELEASE:
		if (now - btn->last_time_ms > btn->debounce_release_ms) {
			...
			if (gpio_get_level(btn->pin) == released_level) {
				...
				// staying in DEBOUNCE_RELEASE For now, let's assume it was
				// noise and proceed to WAIT_FOR_DOUBLE check. A more robust FSM
				// might have a dedicated "bounce" state or re-evaluate.
				btn->state =
		...
		do {
			...
			if (click != BUTTON_NONE_CLICK) {
				...
				if (xQueueSend(btn->output_queue, &local_event,
					...
					ESP_LOGD(TAG, "Button %d: click %d sent to queue", btn->pin,
							 click);
				} else {
...
button_t *button_create(const button_config_t *config,
	...
	btn->timeout_ms =
		config->very_long_click_ms * 2; // Or a more suitable calculation
	...
	if (config->active_low) {
		...
		io_conf.intr_type =
			GPIO_INTR_NEGEDGE; // Interrupt on press (falling edge)
	} else {				   // Active high
		io_conf.pull_up_en = GPIO_PULLUP_DISABLE;
		io_conf.pull_down_en = GPIO_PULLDOWN_ENABLE;
		io_conf.intr_type =
			GPIO_INTR_POSEDGE; // Interrupt on press (rising edge)
	}

	esp_err_t gpio_err = gpio_config(&io_conf);
	if (gpio_err != ESP_OK) {
		ESP_LOGE(TAG, "GPIO config failed for pin %d: %s", config->pin,
	...
	BaseType_t res =
		xTaskCreate(button_task, "button_task", BUTTON_TASK_STACK_SIZE, btn, 10,
					&btn->task_handle);
	if (res != pdPASS) {
	...
	ESP_LOGI(TAG, "Botão criado no pino %d (Active: %s)", config->pin,
