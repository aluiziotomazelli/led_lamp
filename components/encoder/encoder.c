...
#define FS_R_CW_FINAL                                                          \
	0x1 // Renamed to avoid conflict if R_CW_FINAL meant something different for
		// half-step
...
#define H_CCW_BEGIN_M                                                          \
	0x5 // Half-step Counter-Clockwise Begin from Mid-point
...
// R_START (0), H_CCW_BEGIN (1), H_CW_BEGIN (2), H_START_M (3), H_CW_BEGIN_M
// (4), H_CCW_BEGIN_M (5)
static const unsigned char ttable_half_step[6][4] = {
	// Pin States (Input AB)
	// 00                  01              10            11 // (Corresponding to
	// (enc->pin_a << 1) | enc->pin_b values)
	{H_START_M, H_CW_BEGIN, H_CCW_BEGIN, R_START}, // Current State: R_START (0)
	{H_START_M | DIR_CCW, R_START, H_CCW_BEGIN,
	 R_START}, // Current State: H_CCW_BEGIN (1)
	{H_START_M | DIR_CW, H_CW_BEGIN, R_START,
	 R_START}, // Current State: H_CW_BEGIN (2)
	{H_START_M, H_CCW_BEGIN_M, H_CW_BEGIN_M,
	 R_START}, // Current State: H_START_M (3)
	{H_START_M, H_START_M, H_CW_BEGIN_M,
	 R_START | DIR_CW}, // Current State: H_CW_BEGIN_M (4)
	{H_START_M, H_CCW_BEGIN_M, H_START_M,
	 R_START | DIR_CCW} // Current State: H_CCW_BEGIN_M (5)
};
...
static inline long map_value(long x, long in_min, long in_max, long out_min,
							 long out_max) {
	if (in_min == in_max) {
		return out_min; // Or out_max, or an average, depending on desired
						// behavior
	}
...
static void encoder_task(void *arg) {
	...
	// are now considered constant after initialization and can be read
	// directly.
	...
	while (1) {
		...
		if (ulTaskNotifyTake(pdTRUE, portMAX_DELAY)) {
			current_pin_states =
				(gpio_get_level(enc->pin_a) << 1) | gpio_get_level(enc->pin_b);
			...
			if (steps != 0) {
				...
				if (enc->acceleration_enabled) { // Directly use the init-only
												 // flag
					uint32_t current_time_ms = esp_timer_get_time() / 1000;
					uint32_t turn_interval_ms =
						current_time_ms - enc->last_step_time_ms;
					if (turn_interval_ms < enc->accel_gap_ms &&
						enc->last_step_time_ms != 0) {
						current_multiplier_val =
							map_value(enc->accel_gap_ms - turn_interval_ms, 1,
									  enc->accel_gap_ms, 1,
									  enc->accel_max_multiplier + 1);
						if (current_multiplier_val < 1)
							current_multiplier_val = 1;
						if (current_multiplier_val > enc->accel_max_multiplier)
							current_multiplier_val = enc->accel_max_multiplier;
						ESP_LOGD(TAG,
								 "Accel: interval %" PRIu32 ", multiplier %d",
					...
					// for consistent timing if acceleration were to be
					// re-enabled (though not possible now) or for other
					// potential future uses of this timestamp.
				...
				if (enc->output_queue) {
					encoder_event_t evt;
					evt.steps = steps;
					if (xQueueSend(enc->output_queue, &evt,
								   pdMS_TO_TICKS(10)) != pdTRUE) {
						ESP_LOGW(TAG,
								 "Failed to send encoder event to queue for "
								 "pins %d, %d",
								 enc->pin_a, enc->pin_b);
					}
				} else {
					ESP_LOGW(TAG,
							 "Output queue not configured for encoder on pins "
							 "%d, %d",
							 enc->pin_a, enc->pin_b);
				}
			}
			ESP_LOGD(
				TAG,
				"Pins: A=%d B=%d -> States: %02X, Rotary State: %02X, "
				"Direction: %02X, Raw Steps: %d",
				gpio_get_level(enc->pin_a), gpio_get_level(enc->pin_b),
				current_pin_states, enc->rotary_state, direction,
				(direction == DIR_CW ? 1 : (direction == DIR_CCW ? -1 : 0)));
		}
	}
}
...
encoder_handle_t encoder_create(const encoder_config_t *config,
	...
	encoder_handle_t enc =
		(encoder_handle_t)calloc(1, sizeof(struct encoder_s));
	if (!enc) {
	...
	if (gpio_cfg_err != ESP_OK) {
		ESP_LOGE(TAG, "Failed to configure GPIOs: %s",
				 esp_err_to_name(gpio_cfg_err));
		free(enc); // Free allocated memory for enc struct
		return NULL;
	}

	// Install ISR service if not already installed
	esp_err_t isr_service_err =
		gpio_install_isr_service(0); // ESP_INTR_FLAG_LEVEL1 by default
	...
	if (gpio_isr_handler_add(enc->pin_a, encoder_isr_handler, enc) != ESP_OK) {
		...
		// gpio_uninstall_isr_service(); // Potentially, but might affect other
		// components
		free(enc);
		return NULL;
	}
	...
	BaseType_t task_created = xTaskCreate(encoder_task, "encoder_task", ENCODER_TASK_STACK_SIZE,
										  enc, 10, &enc->task_handle);
...
esp_err_t encoder_delete(encoder_handle_t enc) {
	...
	ESP_LOGI(TAG, "Deleting encoder for pins A:%d, B:%d", enc->pin_a,
			 enc->pin_b);
	...
	if (enc->task_handle) {
		...
		ESP_LOGD(TAG, "Encoder task deleted for pins A:%d, B:%d", enc->pin_a,
				 enc->pin_b);
	}

	// 2. Remove ISR handlers
	// It's safe to call remove even if not added or already removed, it will
	// return ESP_ERR_INVALID_STATE.
	esp_err_t err_a = gpio_isr_handler_remove(enc->pin_a);
	if (err_a == ESP_OK) {
		ESP_LOGD(TAG, "ISR handler removed for pin_a %d", enc->pin_a);
	} else if (err_a !=
			   ESP_ERR_INVALID_STATE) { // ESP_ERR_INVALID_STATE means it was
										// not attached or already removed
		ESP_LOGW(TAG, "Error removing ISR handler for pin_a %d: %s", enc->pin_a,
	...
	if (err_b == ESP_OK) {
		...
		ESP_LOGW(TAG, "Error removing ISR handler for pin_b %d: %s", enc->pin_b,
				 esp_err_to_name(err_b));
	}

	// 3. GPIO uninstall ISR service is generally not called here as it's a
	// global service.

	// 4. Mutex Deletion is removed.

	// 5. Free memory
	ESP_LOGI(TAG, "Encoder memory freed for pins A:%d, B:%d", enc->pin_a,
			 enc->pin_b);
	free(enc);
