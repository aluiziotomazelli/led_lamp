 ...
 *          from a queue, applies color correction, and sends it to physical
 * LEDs. Supports both RGB and HSV color modes with hardware acceleration.
 *
 * @author Your Name
 * @date 2024-03-15
 * @version 1.0
...
static esp_err_t configure_led_strip(void) {
		...
		.color_component_format =
			{
				.format =
					{
						.g_pos = LED_STRIP_GREEN_POS,
						.r_pos = LED_STRIP_RED_POS,
						.b_pos = LED_STRIP_BLUE_POS,
	...
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "Failed to create RMT LED strip object: %s",
				 esp_err_to_name(err));
		return err;
	}
...
void led_driver_init(QueueHandle_t input_queue) {
	...
	if (load_err != ESP_OK) {
		ESP_LOGW(TAG,
				 "Failed to load static data, using defaults for correction. "
				 "Error: %s",
				 esp_err_to_name(load_err));
		// Defaults are loaded by the manager, so we can proceed
	}
	led_driver_set_correction(static_data.correction_r,
							  static_data.correction_g,
							  static_data.correction_b);
	...
	// Pin the driver task to Core 1. This is a hard real-time task that drives
	// the RMT peripheral. Pinning it to the application core (Core 1) prevents
	// jitter from the Wi-Fi stack and other system tasks running on Core 0.
	BaseType_t task_created = xTaskCreatePinnedToCore(
		led_driver_task, "LED_DRV_T", LED_DRIVER_TASK_STACK_SIZE, NULL,
