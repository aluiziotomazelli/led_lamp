...
static void led_driver_task(void *pv) {
	...
	while (1) {
		...
		if (xQueueReceive(q_pixels_in, &strip_data, portMAX_DELAY) == pdTRUE) {
            ...
            // Loop through all the pixels, apply color correction, and set them.
			for (uint16_t i = 0; i < strip_data.num_pixels; i++) {
                ...
                if (strip_data.mode == COLOR_MODE_HSV) {
                    ...
                    hsv_to_rgb_spectrum_deg(hsv.h, hsv.s, hsv.v, &final_rgb.r, &final_rgb.g, &final_rgb.b);
                } else {
                    final_rgb = strip_data.pixels[i].rgb;
                }

                // Apply color correction
                final_rgb.r = (uint8_t)(((uint16_t)final_rgb.r * g_correction_r) / 255);
                final_rgb.g = (uint8_t)(((uint16_t)final_rgb.g * g_correction_g) / 255);
                final_rgb.b = (uint8_t)(((uint16_t)final_rgb.b * g_correction_b) / 255);

                // Set the pixel color on the strip
                err = led_strip_set_pixel(led_strip_handle, i, final_rgb.r, final_rgb.g, final_rgb.b);
                if (err != ESP_OK) {
